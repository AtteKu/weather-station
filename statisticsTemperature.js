async function fetchWeatherData(latitude, longitude) {
    ...
    try {
      ...
      /*console.log(weatherData.hourly.temperature2m.length); */
  
      let tempData = [];
        
      for (let i = 0; i < weatherData.hourly.temperature2m.length; i++) {
          
        tempData.push(weatherData.hourly.temperature2m[i]);
      }

      /*console.log(tempData);*/ 

      const sum = tempData.reduce((acc, val) => acc + val, 0);
      const averageTemperature = sum / tempData.length;

      const medianTemperature = getMedian(tempData);

      const TemperatureMode = getMode(tempData);
    
      const TemperatureRange = getRange(tempData);

      const TemperatureSD = getStandardDeviation(tempData);

      const minAndMaxTemperature = getMinMax(tempData);
      const minTemperature = minAndMaxTemperature[0];
      const maxTemperature = minAndMaxTemperature[1];

      /*console.log(averageTemperature);
      console.log(medianTemperature);
      console.log(TemperatureMode);*/
      /*console.log(TemperatureRange); */
      /*console.log(TemperatureSD);*/
      console.log(minAndMaxTemperature);

      document.querySelector("#average-span").innerHTML = ` <strong>${averageTemperature.toFixed(1)} °C</strong>`;

      document.querySelector("#median-span").innerHTML = ` <strong>${medianTemperature.toFixed(1)} °C</strong>`;

      const modeSpan = document.querySelector("#mode-span");
        if (Array.isArray(TemperatureMode)) {
      modeSpan.innerHTML = TemperatureMode.map(val => ` <strong>${val} °C</strong>`).join('<strong>, </strong>');
      } else {
      modeSpan.textContent = TemperatureMode;
      }

      document.querySelector("#range-span").innerHTML = ` <strong>${TemperatureRange.toFixed(1)} °C</strong>`;

      document.querySelector("#sd-span").innerHTML = ` <strong>${TemperatureSD.toFixed(1)} °C</strong>`;

      document.querySelector("#minmax-span").innerHTML = ` <strong>${minTemperature} °C, ${maxTemperature} °C</strong>`;
...
function getStandardDeviation(arr) {
  ...
  const averageTemperature = sum / arr.length;
  const square = [];

  for (let i = 0; i < arr.length; i++) {
    let deviation = (arr[i] - averageTemperature)
